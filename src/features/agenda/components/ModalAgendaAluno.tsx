...
import { X, Save, Plus, Trash2, Clock, Bus, CalendarDays, Loader2 } from "lucide-react";
...
export function ModalAgendaAluno({ alunoId, alunoNome, isOpen, onClose, rotasDisponiveis }: ModalAgendaAlunoProps) {
  ...
  const handleAddHorario = (dia_semana: DiaSemana, tipo_trajeto: TipoTrajeto) => {
    setAgendamentos((prev) => [
      ...prev,
      {
        aluno_id: alunoId,
        rota_id: rotasDisponiveis[0]?.id || "",
        dia_semana,
        tipo_trajeto,
  ...
  const handleUpdateItem = <K extends keyof AgendamentoRota>(index: number, field: K, value: AgendamentoRota[K]) => {
    setAgendamentos((prev) => prev.map((item, i) => (i === index ? { ...item, [field]: value } : item)));
  };
  ...
  const handleSave = async () => {
    try {
      ...
      await agendamentoRotasService.syncAgendamentos(alunoId, {
        agendamentos: agendamentos.map(({ rota_id, dia_semana, tipo_trajeto, horario }) => ({
          rota_id,
          dia_semana,
          tipo_trajeto,
          horario,
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-card border border-border/80 w-full max-w-4xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex flex-col gap-2 p-6 border-b border-border/60 bg-muted/30 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button variant="ghost" size="icon" onClick={onClose} className="rounded-xl h-10 w-10 text-muted-foreground hover:text-foreground hover:bg-muted">
            <X className="size-5" />
          </Button>
        </div>
                ...
                <div key={dia.value} className="bg-muted/20 border border-border/60 rounded-2xl p-5 transition-all hover:bg-muted/40">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {[
                        { tipo: "ENTRADA" as TipoTrajeto, label: "Ida (Entrada)", style: "bg-emerald-500/10 text-emerald-500 border-emerald-500/20 hover:bg-emerald-500/20" },
                        { tipo: "SAIDA" as TipoTrajeto, label: "Volta (Saída)", style: "bg-amber-500/10 text-amber-500 border-amber-500/20 hover:bg-amber-500/20" },
                      ].map(({ tipo, label, style }) => (
                        <Button
                          key={tipo}
                          type="button"
                          variant="outline"
                          size="sm"
                          onClick={() => handleAddHorario(dia.value, tipo)}
                          className={`h-8 text-xs font-semibold rounded-xl ${style}`}
                        >
                          <Plus className="size-3.5 mr-1" /> {label}
                        </Button>
                      ))}
                    </div>
                  </div>
                  ...
                  {agendamentosDia.length === 0 ? (
                    <p className="text-xs text-muted-foreground/70 italic py-2">Nenhum transporte configurado para este dia da semana.</p>
                  ) : (
                    <div className="space-y-2.5">
                      {agendamentosDia.map((item) => (
                        <div key={item.originalIndex} className="flex flex-col sm:flex-row items-center gap-3 bg-card p-3 rounded-xl border border-border/80 shadow-xs">
                          <span className={`text-[10px] uppercase font-bold px-2.5 py-1 rounded-lg w-full sm:w-auto text-center ${
                            item.tipo_trajeto === "ENTRADA" ? "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400" : "bg-amber-500/15 text-amber-600 dark:text-amber-400"
                          }`}>
                            {item.tipo_trajeto}
                          </span>
                          ...
                          <div className="flex-1 w-full flex items-center gap-2">
                            ...
                            <Select value={item.rota_id} onValueChange={(val) => handleUpdateItem(item.originalIndex, "rota_id", val)}>
                          ...
                          <Button
                            ...
                            variant="ghost"
                            size="icon"
                            onClick={() => setAgendamentos((prev) => prev.filter((_, i) => i !== item.originalIndex))}
        ...
        <div className="p-5 border-t border-border/60 bg-muted/30 flex items-center justify-end gap-3">
          ...
          <Button onClick={handleSave} disabled={saving} className="rounded-xl h-11 px-6 font-bold shadow-lg shadow-primary/25">
